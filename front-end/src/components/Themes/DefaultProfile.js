              ...
              {props.user.education.map(school => {
                return(
                  <div key={`education_${school.id}`} style={experienceBox}>
                    ...
                    <h5 style={titleColor}><i className='fas fa-graduation-cap' style={{marginRight:'5px'}}/>{school.school}</h5>
                    <h6 style={textColor}><i className='fas fa-calendar-alt' style={{marginRight:'5px'}}/> {`${dateHelper(school.start_date)} — ${school.end_date ? dateHelper(school.end_date) : 'Present'}`}</h6>
                  </div>
                )
              })}
            </div>
